...
/* То же для дня недели: надпись повёрнута, и высоту строки для неё держит
   не кегль, а ширина колонки дня — в неё крупный кегль умещается свободно
   (см. .day в tv.css). Упереться надпись может в другое: она идёт вдоль
   всех пар дня, а их за неделю бывает и три, и две — «Понедельник» в
   крупный кегль по длине тогда не влезает.

   Нижняя граница здесь куда ниже прочих, и намеренно: у повёрнутой надписи
   нет многоточия, обрезается она с обоих концов разом и читается как брак.
   Мелкий день недели лучше обрезанного — тем более что колонка эта
   служебная, а рядом с ней всё равно стоят даты. Взята она с запасом на
   неделю из двух пар: в такой «Понедельник» занимает всю колонку насквозь.
   Ширина буквы у разных шрифтов разная — на телевизоре свой, — поэтому
   граница именно с запасом, а настоящий кегль подберёт замер. */
var DAY_MAX = 0.78;
var DAY_MIN = 0.22;

/* Время начала пары. Пять знаков моноширинного шрифта в колонке пары —
   это около трёх кеглей ширины, а свободного места там 1.78 высоты строки
   (2.6 всей колонки минус колонка номера, промежуток и отступы), откуда и
   верхняя граница. TIME_MIN — прежний кегль. */
var TIME_MAX = 0.52;
var TIME_MIN = 0.41;
...
function draw() {
  ...
  /* Подбираемые кегли возвращаем к исходным: прошлая отрисовка могла их
     опустить под другую ширину колонки, и без сброса они бы так и остались
     мелкими — вниз fitLabels() ходит, вверх нет. */
  els.board.style.setProperty('--headk', String(HEAD_MAX));
  els.board.style.setProperty('--dayk', String(DAY_MAX));
  els.board.style.setProperty('--timek', String(TIME_MAX));
  ...
  /* Подбор кеглей — последним: все они считаются от --rh, а её настоящее
...
/* ── Подбор кегля по факту ──────────────────────────────────
   Названия групп, дни недели и время начала набраны крупнее прочей
   таблицы: в своих ячейках им есть куда расти. Но сколько именно места
   в ячейке, известно лишь после отрисовки — ширина колонки зависит от
   числа групп, длина колонки дня от числа пар в нём, — а какой ширины
   выйдет надпись, зависит ещё и от шрифта, который на телевизоре свой.
   Поэтому кегли не рассчитываются, а подбираются: меряем готовую
   таблицу и опускаем множитель ровно во столько раз, во сколько
   надписи не хватило места. Надписи набраны в одну строку, их размер
   идёт за кеглем почти пропорционально, и одного-двух проходов хватает.

   Кегль у каждого рода надписей общий на всю таблицу, а не свой у каждой
   ячейки: разнобой в одинаковых с виду ячейках заметен куда сильнее, чем
   недобранная пара пунктов у коротких надписей. Ниже нижней границы не
   опускаемся — там кегль сравнивается с прочей таблицей, и выигрыш всё
   равно исчерпан; дальше надпись обрезается, как обрезалась и раньше. */
function fitLabels() {
  shrink('.head--group', '--headk', HEAD_MAX, HEAD_MIN, tightText);
  shrink('.slot__time', '--timek', TIME_MAX, TIME_MIN, tightText);
  shrink('.day', '--dayk', DAY_MAX, DAY_MIN, tightTurned);
}

function shrink(sel, name, max, min, tight) {
  var nodes = els.board.querySelectorAll(sel);
  if (!nodes.length) return;

  var k = max;
  for (var pass = 0; pass < 2; pass++) {
    var worst = 1;
    for (var i = 0; i < nodes.length; i++) worst = Math.min(worst, tight(nodes[i]));
    if (worst > 0.995) return;               /* всё поместилось */
    k = Math.max(min, k * worst);
    els.board.style.setProperty(name, String(k));
    if (k <= min) return;
  }
}
...
/* Повёрнутая надпись дня. Меряем не переполнение ячейки, а саму надпись:
   она выключена по центру, и половина, вылезшая вверх, в scrollHeight не
   попадает — переполнение по такому замеру вышло бы вдвое меньше
   настоящего. Ограничений тут два, длина и толщина строки, и берётся
   то из них, которого не хватает сильнее. Поворот на размеры не влияет:
   он ровно на пол-оборота. */
function tightTurned(el) {
  var box = el.firstElementChild;
  if (!box) return 1;
  var r = box.getBoundingClientRect();
  var t = 1;
  if (r.height > el.clientHeight + 1) t = Math.min(t, el.clientHeight / r.height);
  if (r.width > el.clientWidth + 1) t = Math.min(t, el.clientWidth / r.width);
  return t;
}
...
var VERSION = 37;   /* поднимайте вместе с ?v= в tv.html */
...
/* Ежеминутной перерисовки таблицы здесь больше нет. Она сохранилась от
   подсветки идущей пары, а та отключена; вторую её работу — снять с
   полосы объявление с истёкшим сроком — делает свой таймер ленты:
   loadTicker() ходит раз в минуту и заканчивается вызовом drawTicker()
   и при удачном чтении, и при неудачном. Смену суток стережёт clock():
   он тикает каждые десять секунд и при новой дате сам зовёт build() и
   draw(). Так что перерисовывать было не для чего.

   А стоила она дорого: тысяча с лишним ячеек в innerHTML, следом fit()
   в два прохода, splitTight() и fitLabels() — все с замерами, то есть с
   принудительным пересчётом раскладки. На одноплатнике это одна длинная
   задача, которая держит главный поток; именно от таких Chrome и пишет
   «страница не отвечает». Таблица по-прежнему собирается заново каждые
   десять минут, в refresh(), — если замер однажды лёг неудачно, дольше
   этого кривая раскладка не проживёт. */
refresh();
loadTicker();
